 ...
 * 
 */
package com.learning.slate.toolkits.madlibs;
 ...
 *
 */
public class MadlibsToolkit extends BaseToolkit
{
	// SLATE SESSION
	...
	// SPECIAL HANDLING
	private static final String TANGIBLE_NOUN_1_NAME = "Noun1";
	private static final String TANGIBLE_NOUN_2_NAME = "Noun2";
	private static final String TANGIBLE_VERB_1_NAME = "Verb1";
	private static final String TANGIBLE_VERB_2_NAME = "Verb2";
	private static final String TANGIBLE_ADJECTIVE_1_NAME = "Adjective1";
	private static final String TANGIBLE_ADJECTIVE_2_NAME = "Adjective2";
	
	private static final List<String> NOUN_1_OPTIONS = new ArrayList<String>();
	static { NOUN_1_OPTIONS.add("shoes");NOUN_1_OPTIONS.add("tires"); NOUN_1_OPTIONS.add("socks");NOUN_1_OPTIONS.add("tshirts");} 
	private static final List<String> NOUN_2_OPTIONS = new ArrayList<String>();
	static { NOUN_2_OPTIONS.add("hat");NOUN_2_OPTIONS.add("phone"); NOUN_2_OPTIONS.add("computer");NOUN_2_OPTIONS.add("window");}
	private static final List<String> VERB_1_OPTIONS = new ArrayList<String>();
	static { VERB_1_OPTIONS.add("run");VERB_1_OPTIONS.add("jump"); VERB_1_OPTIONS.add("sneeze");VERB_1_OPTIONS.add("crumble");} 
	private static final List<String> VERB_2_OPTIONS = new ArrayList<String>();
	static { VERB_2_OPTIONS.add("sing");VERB_2_OPTIONS.add("draw"); VERB_2_OPTIONS.add("cry");VERB_2_OPTIONS.add("sweat");} 
	private static final List<String> ADJECTIVE_1_OPTIONS = new ArrayList<String>();
	static { ADJECTIVE_1_OPTIONS.add("slimy");ADJECTIVE_1_OPTIONS.add("fat"); ADJECTIVE_1_OPTIONS.add("slow");ADJECTIVE_1_OPTIONS.add("crooked");} 
	private static final List<String> ADJECTIVE_2_OPTIONS = new ArrayList<String>();
	static { ADJECTIVE_2_OPTIONS.add("colorful");ADJECTIVE_2_OPTIONS.add("stinky"); ADJECTIVE_2_OPTIONS.add("rough");ADJECTIVE_2_OPTIONS.add("dumb");} 

	private static final Map<Integer,TangibleAction> TANGIBLE_ACTION_MAP = new HashMap<Integer,TangibleAction>();
	static {
		TANGIBLE_ACTION_MAP.put(5, new TangibleAction(TANGIBLE_ADJECTIVE_1_NAME, new Point(250, 230), ADJECTIVE_1_OPTIONS));
		TANGIBLE_ACTION_MAP.put(4, new TangibleAction(TANGIBLE_VERB_1_NAME,      new Point(405, 740), VERB_1_OPTIONS));
		TANGIBLE_ACTION_MAP.put(3, new TangibleAction(TANGIBLE_ADJECTIVE_2_NAME, new Point(405, 360), ADJECTIVE_2_OPTIONS));
		TANGIBLE_ACTION_MAP.put(2, new TangibleAction(TANGIBLE_NOUN_1_NAME,      new Point(490, 740), NOUN_1_OPTIONS));
		TANGIBLE_ACTION_MAP.put(1, new TangibleAction(TANGIBLE_VERB_2_NAME,      new Point(565, 590), VERB_2_OPTIONS));
		TANGIBLE_ACTION_MAP.put(0, new TangibleAction(TANGIBLE_NOUN_2_NAME,      new Point(720, 390), NOUN_2_OPTIONS));
	}
	
	private static class TangibleAction {
		private String name;
		...
		public TangibleAction(String name, Point textLocation, List<String> textOptions) {
			this.name = name;
		...
		public void decrement() {
			if (currentTextCounter > 0) {
				currentTextCounter--;
			} else {
				currentTextCounter = textOptions.size() - 1;
			}
		}
		
		public void increment() {
			if (currentTextCounter < (textOptions.size() - 1)) {
				currentTextCounter++;
			} else {
		...
		public String getCurrentText() {
			return textOptions.get(currentTextCounter);
		}
	}
	
	private TangibleAction getTangibleActionFromCoordinates(SlateObject slateObject) {
		PointF projectionPoint = slateComponent.getProjectionPoint(slateObject);
		float projectedX = projectionPoint.getX();
		float projectedY = projectionPoint.getY();
		if (projectedX > 128) {
			return null;
		}
		int index = (int) Math.floor(projectedY / 128f);
		return TANGIBLE_ACTION_MAP.get(index);
	}
	
	public void processTangibleEvent(SlateObject slateObject, TangibleAction tangibleAction, TuioAction tuioAction, 
			float deltaAngle, float deltaX, float deltaY) {
	
		if (TuioAction.Add.equals(tuioAction)) {
			tangibleAction.activate();
		}
		else if (TuioAction.Remove.equals(tuioAction)) {
			tangibleAction.deActivate();
		}
			...
			{
				tangibleAction.decrement();
			}
			else if (rotateRight(deltaAngle))
			{
				tangibleAction.increment();
			}
		}
	}		
	...
	public void processEvent(SlateObject slateObject, Action action,
		...
		TangibleAction tangibleAction = getTangibleActionFromCoordinates(slateObject);
		if ((BaseStateEnum.Run1.equals(state) || BaseStateEnum.Run2.equals(state)) && (tangibleAction != null))
		{
			processTangibleEvent(slateObject, tangibleAction, tuioAction, deltaAngle, deltaX, deltaY);
			//action = slateComponent.getAction(ACTION_TUTORIAL);
			return;
		}
		else if (action == null)
	...
	public void updateGraphics(Graphics g)
	{
		...
		try
		{
			...
			{
				BaseStateEnum subState = BaseStateEnum.valueOf(networkLogger.getNetworkSubState().name());
			...
			// draw tangibleAction text
			drawTangibleActionText(g2);
	...
	*/
	...
	private void drawTangibleActionText(Graphics2D g2) {
		
		for (Entry<Integer,TangibleAction> entry : TANGIBLE_ACTION_MAP.entrySet()) {
			TangibleAction tangibleAction = entry.getValue();
			if (tangibleAction.isActive()) {
				Point textLocation = tangibleAction.getTextLocation();
				drawTextAt(
						g2,
						tangibleAction.getCurrentText(),
						Color.orange,
						TextType.Dialog_30,
						(int) textLocation.getX(),
						(int) textLocation.getY());
			}
		}
	...
	{
		drawTextAt(g2, text, Color.black, textGroup, 400, 600);
	}
	
	private void drawTextAt(Graphics2D g2, String text, Color color, TextType textGroup, int x, int y)
	{
		AffineTransform saveXform = g2.getTransform();
